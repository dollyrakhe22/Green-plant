...
const loadTree = (id) => {
  ...
  fetch(url)
    .then((res) => res.json())
    .then((data) => displayPlantCards(data.data));
};
  ...
  for (let card of cards) {
    ...
    cardDiv.innerHTML = `
      <div class="bg-white rounded-xl p-4 flex flex-col h-[400px] shadow-sm">
        ...
        <p class="overflow-hidden h-16 mb-5">${card.short_description}</p>
...
const displayCategories = (categories) => {
  ...
  for (let category of categories) {
    ...
    btnDiv.innerHTML = `
      <button class="px-5 py-2 font-semibold text-[#1F2937] hover:bg-green-900 hover:text-white"
        onclick="loadTree('${category.id}')">${category.category}
      </button>
    `;
